...
client = TelegramClient("bot", api_id=29872536, api_hash="65e1f714a47c0879734553dc460e98d6").start(bot_token=BOT_TOKEN)
...
@client.on(events.NewMessage(pattern=r'^/start(@iskdlapaldnskqlpsdlksbot)?$'))
...
@client.on(events.NewMessage(pattern=r'^/rest(@iskdlapaldnskqlpsdlksbot)?(?:\s+(.+))?$'))
...
@client.on(events.NewMessage(pattern=r'^/insta(@iskdlapaldnskqlpsdlksbot)?(?:\s+(.+))?$'))
...
async def fetch_instagram_profile(event, username):
    ...
    try:
        ...
        result = (
            ...
            f"<blockquote><b>ID:</b> {user_id}<br>\n"
